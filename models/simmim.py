...
class Block(nn.Module):
    ...
    def forward(self, x, rel_pos_bias=None):
        if self.gamma_1 is None:
            skip1 = x
            x = self.norm1(x)
            x, attn, attn_ft = self.attn(x, rel_pos_bias=rel_pos_bias)
...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for blk in self.blocks:
            x1, x2, attn, attn_ft = blk(x, rel_pos_bias=rel_pos_bias)
            x = x2
...
def vit_base(cfg="configs/simmim_pretrain__vit_base__img224__800ep.yaml",
             opts=None,
             local_rank=0,
             name="simmim_vit_b"):    
    import argparse
    from models.simmim_config import get_config
    from models.simmim import build_vit
...
def load_state_dict(path):
    ...
    prefix = 'encoder.'  # for default pretrained model
    checkpoint_model = {k.replace(prefix, ''): v for k, v in checkpoint_model.items() if k.startswith(prefix) and not k.endswith("mask_token")}
